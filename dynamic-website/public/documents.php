<?php
    require "../src/isAuth.php";
?>

<!DOCTYPE html>
<html lang="el">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0"> <!-- mobile first -->    
        <meta http-equiv="X-UA-Compatible" content="ie=edge">
        <title>Έγγραφα μαθήματος</title>
        <!--Stylesheets-->
        <link rel="stylesheet" type="text/css" media="screen" href="../css/style.css" />
        <!--Fonts & icons-->
        <link rel="shortcut icon" type="image/ico" href="../images/favicon.ico">
        <link href='https://fonts.googleapis.com/css?family=Inter' rel='stylesheet'>
        <link href="../assets/fontawesome/css/fontawesome.css" rel="stylesheet">
        <link href="../assets/fontawesome/css/brands.css" rel="stylesheet">
        <link href="../assets/fontawesome/css/solid.css" rel="stylesheet">
    </head>

    <body>
        <nav class="sidebar">
            <a href="./introduction.php"><i class="fas fa-home"></i> Αρχική σελίδα</a>
            <a href="./announcement.php"><i class="fas fa-bullhorn"></i> Ανακοινώσεις</a>
            <a href="./communication.php"><i class="fas fa-comment-alt"></i> Επικοινωνία</a></li>
            <a class="active" href="#"><i class="fas fa-file-alt"></i> Έγγραφα μαθήματος</a></li>
            <a href="./homework.php"><i class="fas fa-pencil-ruler"></i> Εργασίες</a></li>
            <?php
                $db = require "../src/connectToDataBase.php"; 
                if($_SESSION['role'] === 'tutor')
                    echo "<a href='./users.php'><i class='fa-solid fa-user-pen'></i> Διαχείριση χρηστών</a></li>";
            ?>
            <a href="../src/logout.php"><i class="fa-solid fa-right-from-bracket"></i> Αποσύνδεση</a></li>
        </nav>

        <main>
            <h1>Έγγραφα μαθήματος</h1>
            <ul>
                <?php 
                    $db = require "../src/connectToDataBase.php"; 
                    $sql = "SELECT id, title, description, file_name FROM documents;";
                    $query = $db->query($sql);
                    
                    if($_SESSION['role'] === 'tutor' ){
                        echo '
                            <li class="list-box doc-box">
                                <h2><a href="./addDocument.php">Προσθήκη νέου εγγράφου</a></h2>
                            </li>';
                
                        while ($x = mysqli_fetch_row($query)) {
                            echo '
                                <li class="list-box doc-box">
                                
                                <h2>
                                    '.$x[1].'
                                    <a href="../src/deleteDocument.php?id='.$x[0].'"><i title="Delete document" 
                                    class="fa-solid fa-trash-can"></i></a>
                                    <a href="./editDocument.php?subject=
                                        '.str_replace(' ', '&nbsp;', $x[2]).'&text='.$x[3].'&id='.$x[0].'"> 
                                        <i class="fa-solid fa-pen-to-square" title="Edit document"></i>
                                    </a>    
                                </h2>
                                <p>
                                    <span class="bold-text">Περιγραφή: </span> 
                                    '.$x[2].'
                                </p>
                                <p><a href="'.$x[3].'">Download</a></p>
                            </li>';
                        }
                    } else {
                        while ($x = mysqli_fetch_row($query)) {
                            echo '
                                <li class="list-box doc-box">
                                    <h2>
                                        '.$x[1].' 
                                    </h2>
                                    <p>
                                        <span class="bold-text">Περιγραφή: </span> 
                                        '.$x[2].'
                                    </p>
                                    <p><a href="'.$x[3].'">Download</a></p>
                                </li>';
                        }
                    }
                ?>
            </ul>

            <div class="top-link">
                <a href="#top">Top <i class="fa-solid fa-arrow-up"></i></a> 
            </div>
        </main>
    </body>
</html>